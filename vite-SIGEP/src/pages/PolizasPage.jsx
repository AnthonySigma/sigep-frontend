...
export default function PolizasPage() {
  ...
  const filtradas = polizas.filter(p => {
    ...
    return texto.includes(busqueda.toLowerCase()) && (filtroEstatus==="Todos" || p.estatusPoliza===filtroEstatus);
  });

  const contadores = {
    Todos: polizas.length, Vigente: polizas.filter(p=>p.estatusPoliza==="Vigente").length,
  ...
  return (
    <div className={`polizas-page ${mounted?"polizas-page--visible":""}`}>
      <div className="polizas-header">
        <div><h1 className="polizas-title">Pólizas</h1><p className="polizas-subtitle">{polizas.length} pólizas registradas</p></div>
        <button className="btn-agregar" onClick={() => navigate("/polizas/crear")}><PlusIcon /> Nueva Póliza</button>
      </div>
